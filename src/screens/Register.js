...
export default function RegisterScreen({ onBack, onRegistered }) {
  ...
  const submit = async () => {
    ...
    try { await register({ name: form.name.trim(), email: form.email.trim(), password: form.password }); Alert.alert('Registration successful', 'Your Heritage Explorer account is ready.', [{ text: 'Continue', onPress: () => onRegistered(form) }]); } catch (error) { Alert.alert('Registration failed', apiMessage(error)); } finally { setIsSubmitting(false); }
  };
